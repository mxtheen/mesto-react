  ...
  return (
      <div className={`popup popup_${props.name} ${props.isOpen ? "popup_opened" : ""}`}>
        <div className={`popup__content popup__content_${props.name}`}>
          ...
          <form className={`popup__form popup__form_${props.name}`} name={`form__${props.name}`} noValidate>
            <h2 className={`popup__title popup__title_${props.name}`}>{props.title}</h2>
            {props.children}
            <button type="submit" className="popup__save-button popup__save-button_edit">{props.buttonContent}</button>
          </form>
        </div>
      </div>
  )
